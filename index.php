      ...
      <div class="container">
        ...
        <div id="navbar" class="navbar-collapse collapse">
          <ul class="nav navbar-nav">
            <li class="active"><a href="http://192.168.3.21"><span class="glyphicon glyphicon-flash"></span> 电子部</a></li>
            <li><a href="mech_page.php"> <span class="glyphicon glyphicon glyphicon-wrench"> </span> 机械部 </a></li>
          </ul>
        </div><!--/.nav-collapse -->
      </div>
    </nav>
...
$db = new MyDB();
 ...
 while($row = $datachunk->fetchArray(SQLITE3_ASSOC) ){
    ...
    if($row['DEPARTMENT'] == 'electric' && $row['DELETE_F'] != 'DEL') {
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
  if (!empty($_POST["id_update"]))
    ...
    $expectdate = date("Y-m-d", strtotime($expectdate));
    ...
    $maildate = date("Y-m-d", strtotime($maildate));
...
?>  <br><br> <input type="button" name="cancle_update" class="btn btn-sm btn-danger" value = "取消更新" onClick="return refresh()"> 
   ...
   <input type="radio" name="id_update_update"<?php echo "checked";?> value="<?php echo $up_idnum?>" hidden = "hidden">
</form>
</td>
<td>
<form name = "update_form" id = "update_form" method="post" action=""> 
请选择编号：<select name="id_update">
<?php  $sql = "SELECT ID , NAME FROM COMPANY WHERE DEPARTMENT = 'electric' AND DELETE_F IS NOT 'DEL' ";
       $idleft = $db->query($sql);
    ...
    <div class="panel panel-success">
        ...
        <div id="collapseOne" class="panel-collapse collapse">
            <div class="panel-body">
                新建表单：请在对应输入框内输入相应信息，点击【提交】按钮进行提交 <br>
                更新表单：请在下拉菜单选择要想要更改物品的名称，点击【更改】按钮，原物品信息会显示在左侧。在左侧表单内进行修改完毕后，点击【提交】更新。或点击【取消更新】取消。<br>
                删除表单：请在下拉菜单选择想要删除物品的名称，点击【删除】并确认后删除改物品。
            </div>
        </div>
    </div>
    </div>
...
<footer class="footer">
      <div class="container">
        <p class="text-muted" align = "center">如有任何问题，请联系郝钧。微信：hj515698566 </p>
      </div>
    </footer>
